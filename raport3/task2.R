...
big_sigma <- matrix(0.5, nrow=450, ncol=450)
diag(big_sigma) <- 1
big_sigma <- big_sigma/500
X <- rmvnorm(500, rep(0, 450), big_sigma)

s <- min(eigen(big_sigma)$values)
s <- min(2*s,1)
sseq <- c(rep(s,450))
V <- 2*diag(sseq)-diag(sseq)%*%solve(big_sigma)%*%diag(sseq)
mu_knock <- X - X%*%solve(big_sigma)%*%diag(sseq)
